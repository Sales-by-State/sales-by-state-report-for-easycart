...
( function ( wp, config ) {
	...
	var API = '/sbsec/v1/report';
	var DIAGNOSTICS = '/sbsec/v1/diagnostics';
	var BACKFILL = '/sbsec/v1/backfill';
	var DEFAULT_PER_PAGE = 25;
	var TEXTDOMAIN = 'sales-by-state-report-for-easycart';
	...
	function ImportStatus( props ) {
		...
		if ( ! diag.table_exists ) {
			return el(
				'div',
				{ className: 'sbsec-notice is-error' },
				el( 'p', { className: 'sbsec-notice__title' }, __( 'The report table is missing.', TEXTDOMAIN ) ),
		...
		return el(
			'div',
			{ className: 'sbsec-importing', role: 'status', 'aria-live': 'polite' },
			el(
				'div',
				{ className: 'sbsec-importing__row' },
				el( 'span', { className: 'sbsec-importing__label' }, __( 'Reading past orders…', TEXTDOMAIN ) ),
				el( 'span', { className: 'sbsec-importing__count' }, pct + '%' )
			),
			el(
				'div',
				{ className: 'sbsec-progress' },
				el( 'div', { className: 'sbsec-progress__bar', style: { width: pct + '%' } } )
			),
			el(
				'p',
				{ className: 'sbsec-importing__hint' },
	...
	function readStatuses( query ) {
		var raw = query && query.sbsecStatuses;
	...
	/**
	 * Compare EasyCart status IDs without number/string mismatches.
	 *
	 * @param {*} a First ID.
	 * @param {*} b Second ID.
	...
	function StatusFilter( props ) {
		...
		return el(
			'div',
			{ className: 'sbsec-filter' },
			el( 'span', { className: 'sbsec-filter__label' }, __( 'Order status', TEXTDOMAIN ) ),
			el( Dropdown, {
				className: 'sbsec-filter__dropdown',
				contentClassName: 'sbsec-status-popover',
				...
				renderToggle: function ( toggleProps ) {
					return el(
						Button,
						{
							className: 'sbsec-filter__toggle',
				...
				renderContent: function () {
					return el(
						'div',
						{ className: 'sbsec-status-popover__list' },
	...
	function Filter( props ) {
		return el(
			'label',
			{ className: 'sbsec-filter' },
			el( 'span', { className: 'sbsec-filter__label' }, props.label ),
			el(
				'select',
				{
					className: 'sbsec-filter__select',
					value: props.value,
					onChange: function ( event ) {
	...
	function SalesByStateReport( props ) {
		...
		var country = query.sbsecCountry || settings.defaultCountry || 'US';
		var year = query.sbsecYear || settings.defaultYear;
		...
		var children = [
			...
			el(
				'div',
				{ key: 'filters', className: 'sbsec-filters' },
				el( Filter, {
					label: __( 'Country', TEXTDOMAIN ),
					value: country,
					options: settings.countries || [],
					onChange: function ( value ) {
						setFilter( { sbsecCountry: value } );
					}
				} ),
				el( Filter, {
					...
					onChange: function ( value ) {
						setFilter( { sbsecYear: value } );
					}
				} ),
				el( StatusFilter, {
					value: statuses,
					onChange: function ( next ) {
						setFilter( { sbsecStatuses: next.join( ',' ) } );
					}
				} )
			)
		];
		...
		if ( data.error ) {
			children.push(
				el(
					'div',
					{ key: 'error', className: 'notice notice-error sbsec-error' },
		...
		return el( 'div', { className: 'woocommerce-analytics__report sbsec-report' }, children );
	}
	...
	function readUrlQuery() {
		var query = {};
		var keys = [ 'sbsecCountry', 'sbsecYear', 'sbsecStatuses', 'orderby', 'order', 'paged', 'per_page' ];
		...
		query.page = found.page || 'sbsec-sales-by-state';
	...
	function mountStandalone() {
		function render() {
			var node = document.getElementById( 'sbsec-root' );

			if ( ! node ) {
				return;
			}

			if ( node.getAttribute( 'data-sbsec-mounted' ) ) {
				return;
			}

			node.setAttribute( 'data-sbsec-mounted', '1' );
