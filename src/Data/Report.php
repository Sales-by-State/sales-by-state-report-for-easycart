...
class Report {
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT shipping_state AS state,
				 ...
				 FROM {$wpdb->prefix}sbsec_order_state
	...
	private function add_labels_and_currency( array $rows, $country ) {
		...
		foreach ( $rows as $index => $row ) {
			...
			$rows[ $index ]['state_name'] = isset( $state_names[ $code ] ) && $state_names[ $code ]
				? $state_names[ $code ]
				: ( '' === $code ? __( 'Unknown', 'sales-by-state-report-for-easycart' ) : $code );
	...
	private function money( $amount ) {
		...
		$dec_sep    = (string) get_option( 'ec_option_currency_decimal_symbol', '.' );
		$thousands  = (string) get_option( 'ec_option_currency_thousands_seperator', ',' );
		$formatted  = number_format( $amount, max( 0, $decimals ), $dec_sep, $thousands );
		$before     = '1' === (string) get_option( 'ec_option_currency_symbol_location', '1' );
